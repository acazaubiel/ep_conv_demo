...
base_regression_icf_98_21 <- tar_read(base_regression_icf_98_21)
base_regression_evh_75_97 <- tar_read(base_regression_evh_75_97)
base_regression_evh_98_21 <- tar_read(base_regression_evh_98_21)
base_regression_evf_75_97 <- tar_read(base_regression_evf_75_97)
base_regression_evf_98_21 <- tar_read(base_regression_evf_98_21)
...
base_geographique <- right_join(fcemetro_shp,base_regression_icf_75_97 %>%
                                 filter(annee==1980) %>%
...
base_geographique_21 <- right_join(fcemetro_shp,base_regression_icf_98_21 %>%
                                 filter(annee==2000) %>%
                                 select(DEP, br_cur_conv),
                                 by=c("code"="DEP"))

tm_shape(base_geographique_21)+
  tm_polygons(c("br_cur_conv"), palette = "RdYlBu")
```
...
```{r}
ggplot(data=base_regression_evh_75_97) +
geom_line(aes(x=annee, y=ecart_moyenne,col=DEP))+
  theme_minimal() +
  facet_wrap(~br_cur_conv, scales="free_y")+
  theme(legend.position="none")+
...
base_geographique <- right_join(fcemetro_shp,base_regression_evh_75_97 %>%
                                 filter(annee==1980) %>%
                                 select(DEP, br_cur_conv),
                                 by=c("code"="DEP"))
